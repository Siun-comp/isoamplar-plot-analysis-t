...
| --- | --- | --- | --- | --- | --- |
| FR-001 | Must | As a user, I want to import PCR Excel data. | The app shall import `.xls` and `.xlsx` files from the browser, parse only the first worksheet, preserve Excel-formatted specimen/reagent display identity with raw cell provenance, and support either replacing the current dataset or appending another file to the current analysis. Within each source, a truly blank later specimen header shall inherit the nearest previous explicit specimen label. A truly blank reagent header or exact trimmed literal `-` shall exclude that physical column from analysis with explicit ignored-warning evidence. | AC-001, AC-002, AC-PCR-001, AC-PCR-015, AC-PCR-016, AC-PCR-025, AC-PCR-047, AC-PCR-048, AC-PCR-061, AC-PCR-062 | Later worksheets are ignored. If every candidate column is excluded, import fails. The first included curve must resolve to a specimen label. An explicit specimen over an excluded reagent column remains an inheritance anchor for later included curves. Reagent headers are never inherited. Formula-backed or non-empty labels such as `R-4` remain included. |
...
| FR-003 | Should | As a user, I want to add a small amount of data without creating a new Excel file. | The app shall support Quick Paste Import: full-table paste using the same structure and specimen-header inheritance contract as Excel input, or single-specimen paste where the user supplies one specimen name and pastes reagent/fluorescence rows. It shall generate a read-only preview with warnings and size/memory summary, then append it to the current analysis or open it as a new analysis. Accepted-shape parsing and import errors shall be contained without mutating the current analysis. The app shall not provide in-app cell editing, source-data correction, custom cycle columns, or spreadsheet-grid editing. | AC-PCR-017, AC-PCR-061, AC-QP-001 through AC-QP-021 | This is paste/import only. Tab-separated tables and unambiguous single-column input are supported. Comma/CSV tables are rejected to prevent silent delimiter corruption. Incorrect pasted data is corrected by changing the textarea source/specimen field and regenerating preview before import. CSV file import remains deferred. Browser-process OOM is outside catchable error guarantees. |
| FR-004 | Must | As a user, I want to review detected PCR structure. | The app shall detect and display row 1 specimens, row 2 reagents, row 3+ fluorescence values, generated cycle values, and import diagnostics. A shared Warning Inspector shall expose source, worksheet, cell/range, raw/display evidence, handling, affected curves, and specimen-inheritance source/target cells where available. | AC-001, AC-002, AC-PCR-001, AC-PCR-019, AC-PCR-048, AC-PCR-061 | Similar names warn only; no auto-merge, rename, or correction UI. The explicit blank-as-previous specimen contract is the only neighboring-header inference. Warning navigation is transient and does not change analysis state. |
...
| FR-010 | Must | As a user, I want fixed Y-axis scales. | The app shall allow automatic, user fixed, user-editable FAM/HEX preset, or plot-area Box zoom Y-axis min/max values. New analyses shall initialize FAM to `-200000..1600000` and HEX to `-100000..600000`. | AC-008, AC-PCR-024, AC-PCR-044, AC-PCR-062 | Fixed bounds persist across filtering and redraws. Preset labels and bounds remain editable per analysis; restored Analysis XLSX values remain authoritative. Previous scale returns one Box zoom step at a time; Auto scale is a separate full automatic reset. X-axis presets remain blank user-defined P1/P2 values. |
...
| FR-017 | Must | As a user, I want to continue a large analysis later. | The app shall expose a top-level analysis save command and import an Analysis XLSX restore file containing the full imported normalized dataset, source/header provenance, warnings and handling, selected/unselected/excluded state, Selection Sets, grouping/collapse state, order, scale presets, style rules, individual overrides including Analysis labels, legend/export settings, common and per-reagent Threshold configuration, analysis name, and export counter. It shall show the last completed snapshot time and whether later changes remain, and shall reject contradictory restore relationships before creating a restored tab. | AC-PCR-033, AC-PCR-034, AC-PCR-036, AC-PCR-037, AC-PCR-041, AC-PCR-048, AC-PCR-049A, AC-PCR-049B, AC-PCR-050, AC-PCR-053, AC-PCR-058, AC-PCR-062, AC-PCR-064, AC-PCR-068 | Analysis XLSX schema 7 is a web-app project/session file, not a native editable Excel chart workbook. Hidden restore JSON is authoritative; visible sheets include per-reagent Threshold and excluded-curve review tables. Schema 6 and earlier migrate with no excluded curves. |
...
| FR-023 | Should | As a user, I want to use one common raw-fluorescence Threshold or exact per-reagent Thresholds and inspect auditable crossing evidence. | The app shall resolve one applied Threshold per selected curve from the active Common or Per reagent mode, calculate complete unmodified X/Y arrays using versioned rule `raw-first-upward-linear-v1`, distinguish observed evidence from the Cycle-axis linear estimate, and keep results in current curve order. Preview and plot-bearing export shall use the same assignment projection. | AC-PCR-056, AC-PCR-057, AC-PCR-058, AC-PCR-062, AC-PCR-064 | Per-reagent settings use exact `reagentId`; similar names are not merged. Unconfigured reagents are excluded and explicitly identified, never reported as ND. Switching modes preserves both configurations. Append reuses exact IDs and leaves new IDs unconfigured. Raw fluorescence remains unmodified. |
| FR-024 | Should | As a user, I want to copy the currently reviewed Threshold summary directly into Excel cells. | The Threshold review panel shall copy the currently filtered results in current curve order as rich HTML and tab-separated clipboard data with specimen, reagent, applied Threshold, numeric estimated crossing Cycle, and result status columns. | AC-PCR-060, AC-PCR-062, AC-PCR-064 | Unavailable estimates remain blank, valid upward crossings use `Positive`, no reached crossing uses `ND`, and multiple crossings remain explicitly review-needed. `결과 상태` is not a clinical decision. |
| FR-025 | Should | As a user, I want to know which app version produced an analysis and reopen an earlier stable runtime when reproducing prior work. | The app shall display its current semantic version, show a user-facing change history, record the app version in Analysis XLSX review metadata, and provide same-origin links to immutable archived T releases. | AC-PCR-063 | Only the current T release is maintained. Archived versions are read-only reproduction tools and may not accept Analysis XLSX files written by newer schemas. A release must archive and hash the prior deployed build before the current version is promoted. |
| FR-026 | Should | As a user, I want quick access to the established eight-color sequence without losing detailed color controls. | Every group and individual color popover shall expose the eight default colors as keyboard-accessible square swatches that commit through the existing color mutation path. | AC-PCR-065 | HEX input, native picker, reset, stable default assignment, presets, preset undo, and custom-color support remain available and unchanged. |
| FR-027 | Must | As a user, I want to remove mistakenly entered curves from the current analysis without destroying source evidence. | The app shall reversibly exclude exact `curveId` values at curve, specimen-group, or reagent-group scope; remove excluded curves from active selection, Plot, Legend, Threshold projection, and selected-data output; and provide exact restore. | AC-PCR-066, AC-PCR-067, AC-PCR-068 | Exclusion never edits or deletes imported X/Y values, provenance, warnings, styles, or Selection Set membership. Duplicate labels require source-aware scope confirmation. Restored curves return unselected. |
