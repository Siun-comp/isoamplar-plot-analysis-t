...
| AC-PCR-058 | FR-017, FR-021, FR-023, IO-106, IO-107, IO-108, D051, D060 | Analysis XLSX schema 6 roundtrips Common/per-reagent Threshold configuration while schema 1-5 migration preserves provenance, Selection Sets, and Common mode. Selected Data XLSX schema 4 preserves raw `PlottedData`, records the exact reagent ID/applied Threshold/rule in result and event sheets, and recognizes schemas 1-4 as output-only. Restore recomputes results and no output mutates raw fluorescence or performs clinical interpretation. | Unit + workbook readback + E2E |
...
| AC-PCR-064 | FR-017, FR-023, FR-024, IO-106, IO-107, IO-108, IO-109, D060 | Common and exact per-reagent modes preserve independent settings across mode switches and tabs. Per-reagent drafts apply atomically, exact reagent IDs reuse settings on append, new IDs remain unconfigured, and replacement resets state. Preview, plot export, review, five-column clipboard, Analysis XLSX schema 6, and Selected Data XLSX schema 4 use one curveId-based Threshold assignment; unconfigured curves are never ND. Multiple visible values use distinct/deduplicated lines and collision-managed labels without changing raw data or axis bounds. | Unit + store + workbook readback + component + fresh Chromium raster/clipboard/restore |
