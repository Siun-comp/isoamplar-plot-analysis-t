...
- Final branch CI run `29156244025` and Pages run `29156323546` passed for promoted product artifact source SHA `eae3281fb8f9bbbd900fab528be3e094b93b555a`, tagged `release-20260711-audit-remediation-r1`. Evidence-only documentation commits may follow without changing the verified dist hash. The initial candidate Pages run `29156159533` failed before build on historical Markdown whitespace and did not deploy; the corrected workflow passed the same full-range formatting gate.
...
## Post-S11 Selection Workflow Extension

- `FR-021` / `AC-PCR-054` / `AC-PCR-055`: Selected Data XLSX exports only the current non-empty rectangular common-X selection in user order, preserves numeric/null fluorescence and all X rows, records source/warning/applied-scale evidence, and carries an output-only hidden role marker.
...
- Deployment status: release commit `60b94148ff95824cfe2cde44ca11f6eab782676c` and annotated `v1.0.0`/`v1.1.0` tags were pushed only to the maintained T repository. Pages run `30802527069` succeeded, and public latest/archive smoke confirmed v1.1.0 version history, FAM/HEX defaults, executable v1.0.0 rollback, and no browser console warning/error.

## v1.2.0 Per-Reagent Threshold and Palette Candidate - 2026-09-02

- Exact assignment: `D060`, `FR-023`, `IO-106`, and `AC-PCR-064` bind optional per-reagent Threshold settings only through stable `reagentId`; missing assignments are excluded and never projected as ND.
- Atomic workflow: every populated/currently-applied reagent draft is validated before `모두 적용`; one invalid row leaves all prior applied values unchanged. Common and per-reagent configurations survive mode switching independently.
- Consistent projection: preview and image export share one marker builder. Equal numeric Thresholds merge into one line; distinct values use reagent-aware color and a collision-managed label rail without altering chart data or legend order.
- Continuity/evidence: Analysis XLSX schema 6 stores both modes and migrates schema 5 to Common. Selected Data XLSX schema 4 records exact reagent ID, applied Threshold, rule ID, unconfigured status, and current curve order while preserving raw `PlottedData`.
- Style UX: `D061` adds the established eight colors as an optional 4-by-2 quick palette in every color popover while retaining HEX input, native picker, group/curve scope, and reset behavior.
- Release evidence: local gates pass at 45 Vitest files / 393 tests, audit 1/1, dependency vulnerabilities 0, Pages-base build, Chromium 13/13, and byte-identical pre/post-Playwright `dist` hash `d4c5651a323f6158ca5323b739121c308a483325ed50f2a8736678b57347c6e0`. Specialist re-audits confirmed the core identity/projection invariants before a usage-limit interruption; the lead completed their two recorded residual checks by adding explicit all-unconfigured workbook evidence and value-preserving long-label coverage. Head `6936093`, tag `v1.2.0`, and Pages run `33558251077` are deployed; public latest/v1.1/v1.0 smoke passed without console warning/error. The user guide is not regenerated for v1.2.0 by user direction.
## v1.3.0 Reversible Analysis Exclusion

| Requirement | Implementation | Verification |
| --- | --- | --- |
| FR-027 exact exclusion/restore | `SelectionState.excludedCurveIds`, store exclusion actions, `DataExclusionDialog` | app-store exact-ID, duplicate-label append, component and Chromium flows |
| AC-PCR-067 duplicate-source safety | source-instance buckets with file/sheet/column confirmation | same-label append regression and browser source-scope check |
| AC-PCR-068 continuation | Analysis schema 7, full `ImportedData`, visible `ExcludedCurves` | schema migration/integrity and workbook roundtrip tests |

Local release evidence: 45 Vitest files / 399 tests, audit 1/1, production dependency vulnerabilities 0, Pages-base build, fresh Chromium 14/14, and byte-identical pre/post-Playwright `dist` hash `925692d6a95657e840b62d413de2e9fbdc50251e32b8897e43ec534f2981cf96`. The browser flow reads downloaded Selected Data XLSX and Analysis XLSX back to verify active-only output versus full-dataset continuation. A 1280x720 visual check covers exact-curve exclusion, restore-unselected, and duplicate-name source-scope selection with no browser warning/error.

Final specialist review: data-integrity review returned GO. QA/release review found stale archive-list and dist-hash evidence; both were closed before promotion. `releaseEvidence.test.ts` now verifies every v1.2.0 archive byte and permits only the recorded three documentation-evidence files between the v1.2.0 tag commit and the final archived source commit.

Deployment evidence: release commit `da8232dcdd89c05fd0334862968e2d816f82e913`, annotated tag `v1.3.0`, and Pages run `33773618844` succeeded. Public latest reported v1.3.0 / Analysis XLSX schema 7; v1.2.0, v1.1.0, and pre-version-history v1.0.0 archives all rendered the maintained product identity with no browser warning/error.
