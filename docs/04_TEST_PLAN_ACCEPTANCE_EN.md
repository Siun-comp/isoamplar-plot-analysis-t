...
| AC-PCR-010 | FR-011, D014 | PNG and JPEG export produce valid images matching current preview selection, scale, style, legend/order state, and export layout, using a white background and analysis-name-based safe filenames. Plot-bearing output keeps a 2400 px-wide raster while applying only export-specific report typography and geometry. | Automated smoke + manual |
...
| AC-PCR-014 | D010, D063 | Retired/default-forbidden UI terms do not appear in user-visible UI: `replicate`, `series`, `solo/isolate`, and hover/solo emphasis wording. The later-approved analysis-scope exclude/restore wording is permitted only for reversible curve visibility management under FR-027 and must not imply source deletion, biological classification, or QC omission. | Automated text scan + manual |
...
| AC-PCR-024 | FR-006, FR-008, FR-009, FR-010, D021, D023, D024, D052 | The maintained product identity is `IsoAmplar Plot Analysis T` with developer credit `Jang Si Un`; preview plot height remains stable when side settings expand; default curve colors remain stable as earlier/later curves are selected; default lines have no markers; individual curve marker overrides support none, circle, triangle, and rect; X P1/P2 and Y FAM/HEX presets are user-editable and selectable only with valid min/max; their applied status uses the current user label rather than internal keys; Fixed mode displays current Auto bounds for editing support. | Unit + component + E2E |
...
| AC-PCR-037 | FR-006, FR-017, D026, D051, D059 | Analysis XLSX contains no native editable Excel chart, and export/import performs no smoothing, normalization, baseline correction, automatic/import-time threshold calculation, Ct/Cq/Tt/Tp naming, or clinical interpretation. Approved user-set raw Threshold configuration is persisted without derived result caches; `Positive`/`ND` remain explicitly non-clinical crossing-status labels. | Unit + package/workbook inspection |
...
| AC-PCR-056 | FR-023, IO-101, IO-108, D051 | One tab-local Threshold supports disabled, draft, applied, revert, clear, append/replace, tab isolation, independent preview visibility and plot-export inclusion toggles, and out-of-Y-range states. Enabled targets show the same silent horizontal line or range annotation without altering Auto/Fixed/P1/P2/Box zoom bounds, calculated results, or custom legend identity. | Unit + component + E2E + raster evidence |
| AC-PCR-057 | FR-023, IO-108, D051 | Versioned `raw-first-upward-linear-v1` scans acquisition order, uses only adjacent finite points satisfying `previous < T && current >= T`, never interpolates across null, gives chronological priority to earlier leading/internal gap uncertainty, preserves every event and source/formula-cache evidence, and distinguishes observed points from the Cycle-axis linear estimate. | Unit + property/edge-case review + component |
| AC-PCR-058 | FR-017, FR-021, FR-023, IO-106, IO-107, IO-108, D051, D060 | Analysis XLSX schema 7 roundtrips Common/per-reagent Threshold configuration and exact exclusions while schema 1-6 migration preserves provenance, Selection Sets, Common mode, and an empty exclusion set. Selected Data XLSX schema 4 preserves raw `PlottedData`, records the exact reagent ID/applied Threshold/rule in result and event sheets, and recognizes schemas 1-4 as output-only. Restore recomputes results and no output mutates raw fluorescence or performs clinical interpretation. | Unit + workbook readback + E2E |
| AC-PCR-059 | FR-011, FR-019, IO-102, D053 | A new analysis defaults Chart image layout to Plot only. Plot-bearing PNG/JPEG/clipboard rendering retains a 1200 x 760 logical surface at 2x output density and enlarges axis labels/titles, margins, major ticks, curve/marker geometry, and both in-range and out-of-range Threshold annotations for approximately 9.5 cm Excel placement. Relative line-width differences and matching Plot + Legend samples remain distinguishable. Preview geometry, raw data, Scale bounds, curve order, and explicit Analysis XLSX layout values remain unchanged. | Unit + component + Playwright raster + manual 9.5 cm inspection |
| AC-PCR-060 | FR-024, IO-109, D054 | The T-edition Threshold review copy icon is keyboard accessible beside the status filter, disables when the filtered projection is empty, and copies only currently visible rows in current curve order. Clipboard HTML and TSV both contain four separate columns; valid estimates are numeric, unavailable estimates are blank, multiple crossings retain review wording, formula-like labels are neutralized, and success/failure feedback is visible without mutating analysis state. | Unit + component + Playwright clipboard readback + visual review |
| AC-PCR-061 | FR-001, FR-003, FR-004, IO-001, IO-003, D055 | For `.xlsx`, BIFF8 `.xls`, and full-table Quick Paste, `S1, blank, S2, blank` resolves to `S1, S1, S2, S2` without changing fluorescence, X values, physical column IDs, or blank-cell provenance. The first included curve must resolve from its own specimen header or a previous explicit physical-column anchor in the same source; a failed initial or append source leaves the current analysis unchanged. Inheritance never crosses source boundaries or applies to reagents, formula-without-cache, or formatted-display-empty headers. Informational evidence identifies both explicit source and inherited target cells, horizontal specimen merges report actual use, and mixed-source Analysis XLSX roundtrips resolved labels plus original provenance. Restore rejects missing, cross-source, nonblank, formula-derived, or non-nearest inheritance evidence and validates a 4,096-column mostly inherited source through indexed relationships. | Unit + fixture + store + workbook readback + E2E |
| AC-PCR-062 | FR-001, FR-010, FR-017, FR-023, FR-024, IO-001, IO-107, IO-109 | Blank/literal-`-` reagent columns are excluded without curve IDs but retain source diagnostics and specimen-anchor semantics; all-excluded input is blocked. Original Excel names drop only the final workbook extension in the default analysis name, Analysis XLSX does not add a duplicate date prefix, new Y presets are editable FAM/HEX defaults, and Threshold UI/clipboard/Selected Data XLSX consistently expose `Positive`/`ND` as non-clinical analysis statuses while preserving raw outcomes. | Unit + fixture + store + workbook readback + E2E |
| AC-PCR-063 | FR-025, IO-110 | The current semantic version is visible and keyboard accessible, the dialog shows current release notes and schema information, the visible Analysis XLSX Settings sheet records the app version, and every listed previous release resolves to a same-origin immutable archive whose files match its SHA-256 manifest. | Unit + archive integrity + production build + Playwright + deployed URL smoke |
| AC-PCR-064 | FR-017, FR-023, FR-024, IO-106, IO-107, IO-108, IO-109, D060 | Common and exact per-reagent modes preserve independent settings across mode switches and tabs. Per-reagent drafts apply atomically, exact reagent IDs reuse settings on append, new IDs remain unconfigured, and replacement resets state. Preview, plot export, review, five-column clipboard, Analysis XLSX schema 7, and Selected Data XLSX schema 4 use one curveId-based Threshold assignment; unconfigured or excluded curves are never ND. Multiple visible values use distinct/deduplicated lines and collision-managed labels without changing raw data or axis bounds. | Unit + store + workbook readback + component + fresh Chromium raster/clipboard/restore |
| AC-PCR-065 | FR-008, FR-026, D061 | Every shared color popover exposes the ordered eight square swatches with visible selected state, color-name/HEX accessible labels, and keyboard activation. A swatch updates only the intended group or curve through the existing mutation path; HEX, picker, reset, presets, undo, and stable append defaults remain unchanged. | Unit + component + desktop browser visual/keyboard regression |
| AC-PCR-066 | FR-027, D063 | Excluding one exact `curveId` removes only that curve from selection, Plot, Legend, Threshold, and selected-data projections while raw X/Y, provenance, warnings, styles, order, and Selection Set membership remain unchanged. Restore returns it unselected. | Unit + store + component + E2E |
| AC-PCR-067 | FR-027, D063 | Duplicate-label group exclusion identifies source file, worksheet, source instance, and column before confirmation. One-source exclusion does not affect same-label curves from another source, and appended same-label curves never inherit prior exclusion. | Store + component + E2E |
| AC-PCR-068 | FR-017, FR-027, IO-106, D063 | Analysis XLSX schema 7 roundtrips the complete imported dataset and exact exclusions, rejects selected/excluded overlap, and includes `ExcludedCurves`. Schemas 1-6 migrate with no exclusions. Selection Sets retain excluded members but apply only active members. | Unit + workbook readback + E2E |
...
| AC-QP-005 | FR-003, IO-003 | The first included curve blocks preview only when its own specimen header is blank and no previous explicit physical-column anchor exists in that paste source; truly blank later specimen headers inherit with information evidence. Blank/whitespace or literal `-` reagent columns are excluded with ignored diagnostics, while unresolved non-shorthand specimen headers, empty fluorescence cells, nonnumeric fluorescence cells, duplicate curve labels, and similar specimen/reagent labels produce diagnostics consistent with the Excel parser policy. | Unit |
...
| AC-QP-010 | FR-003, FR-006, FR-023, D037, D051 | Paste import applies no smoothing, normalization, baseline correction, log transform, averaging, automatic/import-time thresholding, Ct/Cq/Tt/Tp calculation, or clinical interpretation. A later explicit user-set raw Threshold uses the same derived calculation path as Excel curves without changing the pasted source data. | Unit + code review |
...
2. Browser-local Excel parsing: test specimen/reagent extraction, within-source specimen forward-fill, first-usable-specimen blocking, source/target provenance, first-sheet-only behavior, `.xls`/`.xlsx` equivalence, generated cycles, nonnumeric values, blank rows/columns, duplicate or similar names, missing headers, uneven lengths, formulas, old `.xls` encodings, protected files, and unsupported files.
...
33. In the T edition, apply a synthetic Threshold, filter the Threshold review rows, use the adjacent copy icon, paste into desktop Excel, and confirm four cells per row with a numeric estimated Cycle, blank unavailable estimate, retained result status, current filtered order, and readable Malgun Gothic 9 pt formatting.
...
## Current Automated Coverage - 2026-08-03
- Final v1.1.0 gate: Vitest 45 files / 379 tests, audit 1/1, fresh Pages-base Chromium 13/13, production dependency vulnerabilities 0, and byte-identical pre/post Playwright `dist`.
- Vitest: parser `.xlsx` / `.xls`, first-sheet-only, invalid first sheet, blank/nonnumeric/formula cells, excluded reagent columns and physical specimen anchors, similar-name warnings, append merge identity, selection tree and Selection Set state, Analysis XLSX schema 1-6 migration/validation, Selected Data XLSX schema 1-4 role compatibility, tab isolation, chart option theme/scale/style, hover behavior, exact per-reagent Threshold assignment, style resolution, export filename generation, plotted-data CSV shape, Selected Data XLSX numeric/null/provenance/formula safety, AnalysisState serialization, Analysis XLSX workbook readback, and tab-routing safety.
...
- Playwright: fresh Pages-base-path production preview, generated `.xlsx` upload/append, Quick Paste boundaries, selection/search, scale/Box zoom, Style/Legend interaction, chart hover/readout, raster/clipboard/network gates, Selection Set switching and overflow, schema-5 Analysis XLSX save/restore, Selected Data XLSX workbook readback, output-role rejection without analysis mutation, and T-only Threshold rich HTML/TSV clipboard readback.
