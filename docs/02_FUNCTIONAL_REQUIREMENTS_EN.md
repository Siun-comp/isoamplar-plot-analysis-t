...
| --- | --- | --- | --- | --- | --- |
| FR-001 | Must | As a user, I want to import PCR Excel data. | The app shall import `.xls` and `.xlsx` files from the browser, parse only the first worksheet, preserve Excel-formatted specimen/reagent display identity with raw cell provenance, and support either replacing the current dataset or appending another file to the current analysis. Within each source, a truly blank later specimen header shall inherit the nearest previous explicit specimen label, while a blank specimen in the first usable curve column shall block that source. | AC-001, AC-002, AC-PCR-001, AC-PCR-015, AC-PCR-016, AC-PCR-025, AC-PCR-047, AC-PCR-048, AC-PCR-061 | Later worksheets are ignored. If the first worksheet is invalid, import fails even if later worksheets are valid. Reagent headers are never inherited. File signature mismatch is diagnostic only until its policy is separately decided. |
...
| FR-024 | Should | As a T-edition user, I want to copy the currently reviewed Threshold summary directly into Excel cells. | The Threshold review panel shall copy the currently filtered results in current curve order as rich HTML and tab-separated clipboard data with specimen, reagent, numeric estimated crossing Cycle, and result status columns. | AC-PCR-060 | This is T-edition only. Unavailable estimates remain blank, multiple crossings remain explicitly review-needed, and `결과 상태` must not be presented as a clinical decision. The action does not mutate analysis state or source data. |
