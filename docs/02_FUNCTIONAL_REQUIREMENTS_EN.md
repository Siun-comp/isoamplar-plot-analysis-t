...
| FR-017 | Must | As a user, I want to continue a large analysis later. | The app shall expose a top-level analysis save command and import an Analysis XLSX restore file containing the full imported normalized dataset, source/header provenance, warnings and handling, selected/unselected state, Selection Sets, grouping/collapse state, order, scale presets, style rules, individual overrides including Analysis labels, legend/export settings, Threshold configuration, analysis name, and export counter. It shall show the last completed snapshot time and whether later changes remain, and shall reject contradictory restore relationships before creating a restored tab. | AC-PCR-033, AC-PCR-034, AC-PCR-036, AC-PCR-037, AC-PCR-041, AC-PCR-048, AC-PCR-049A, AC-PCR-049B, AC-PCR-050, AC-PCR-053, AC-PCR-058, AC-PCR-062 | Analysis XLSX schema 5 is a web-app project/session file, not a native editable Excel chart workbook. Hidden restore JSON is authoritative; visible review sheets are secondary. Opening original Excel removes only the final `.xls`/`.xlsx` from the default analysis name without changing source provenance. Saving uses `<sanitizedAnalysisName>_analysisN.xlsx` without adding another date prefix. Older supported schemas migrate before semantic validation. |
...
| FR-023 | Should | As a user, I want to set one raw-fluorescence Threshold per analysis and inspect auditable crossing evidence. | The app shall calculate current selected curves from their complete unmodified X/Y arrays using versioned rule `raw-first-upward-linear-v1`, distinguish observed evidence from the Cycle-axis linear estimate, preserve gap/start/multiple-event outcomes, and keep results in current curve order. Preview visibility and plot-bearing export inclusion shall be independent controls that do not alter calculation. | AC-PCR-056, AC-PCR-057, AC-PCR-058, AC-PCR-062 | Raw fluorescence only; no correction, smoothing, normalization, automatic recommendation, Ct/Cq/Tt/Tp naming, or clinical interpretation. For the user's analysis workflow, a valid upward crossing is displayed as `Positive` and no reached crossing as `ND`; these are Threshold analysis statuses, not clinical positive/negative decisions. Analysis XLSX persists configuration, not derived result caches. |
| FR-024 | Should | As a user, I want to copy the currently reviewed Threshold summary directly into Excel cells. | The Threshold review panel shall copy the currently filtered results in current curve order as rich HTML and tab-separated clipboard data with specimen, reagent, numeric estimated crossing Cycle, and result status columns. | AC-PCR-060, AC-PCR-062 | Unavailable estimates remain blank, valid upward crossings use `Positive`, no reached crossing uses `ND`, and multiple crossings remain explicitly review-needed. `결과 상태` is not a clinical decision. The action does not mutate analysis state or source data. |
