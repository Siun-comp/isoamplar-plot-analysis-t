...
| AC-PCR-058 | FR-017, FR-021, FR-023, IO-106, IO-107, IO-108, D051 | Analysis XLSX schema 5 roundtrips Threshold configuration while schema 1-4 migration preserves provenance and Selection Sets. Selected Data XLSX schema 3 preserves raw `PlottedData`, adds formula-safe result/event sheets plus a user-facing Analysis status, and recognizes schemas 1-3 as output-only. Restore recomputes results and no output mutates raw fluorescence or performs clinical interpretation. | Unit + workbook readback + E2E |
...
- Vitest: parser `.xlsx` / `.xls`, first-sheet-only, invalid first sheet, blank/nonnumeric/formula cells, excluded reagent columns and physical specimen anchors, similar-name warnings, append merge identity, selection tree and Selection Set state, Analysis XLSX schema 1-5 migration/validation, Selected Data XLSX schema 1-3 role compatibility, tab isolation, chart option theme/scale/style, hover behavior, style resolution, export filename generation, plotted-data CSV shape, Selected Data XLSX numeric/null/provenance/formula safety, AnalysisState serialization, Analysis XLSX workbook readback, and tab-routing safety.
