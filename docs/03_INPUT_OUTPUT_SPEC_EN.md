...
- Analysis XLSX schema 7 is a web-app restore file containing the full imported dataset and settings, including exact excluded curve IDs, Analysis labels, Selection Sets, Common/per-reagent Threshold configuration, and legend/export settings; it is separate from plotted-data CSV and report-style chart workbooks.
...
| IO-106 | Analysis XLSX export | Export schema 7 with the complete imported dataset, exact exclusion state, and common/per-reagent Threshold mode, drafts, applied values, enabled states, rule IDs, and independent preview/export visibility. Visible `ReagentThresholds` and `ExcludedCurves` sheets support review; hidden restore JSON remains authoritative. Schema 6 restores with no excluded curves. Derived results are recomputed from raw data. | FR-017, FR-023, FR-027 | AC-PCR-033, AC-PCR-034, AC-PCR-037, AC-PCR-058, AC-PCR-064, AC-PCR-068 |
| IO-107 | Selected Data XLSX export | Export the current non-empty rectangular common-X selection in current user order as schema 4. `ThresholdResults` records the exact applied Threshold per curve and explicit `unconfigured` rows; `ThresholdEvents` records reagent ID/name, applied Threshold, rule ID, and source evidence. Numeric fluorescence remains numeric, null remains blank, and all common-X rows are exported regardless of visible scale. | FR-021, FR-023 | AC-PCR-054, AC-PCR-055, AC-PCR-058, AC-PCR-062, AC-PCR-064 |
...
| IO-109 | Threshold Excel clipboard projection | Copy only the Threshold review rows visible under the current status filter, preserving current curve order. Emit rich HTML and TSV with headers `검체`, `시약`, `적용 Threshold`, `추정 교차 Cycle`, `결과 상태`; keep both Threshold and valid estimate numeric, leave unavailable estimates blank, and retain non-clinical status wording. | FR-024 | AC-PCR-060, AC-PCR-062, AC-PCR-064 |
...
## Analysis Exclusion and Restore Rules

- Exclusion identity is the exact persisted `curveId`; specimen/reagent text is display and scope-discovery metadata only.
- Individual exclusion targets one curve. Group exclusion resolves the currently active descendant curve IDs at confirmation time.
- When one displayed group spans multiple source instances, the confirmation UI separates each source by file, worksheet, and physical column and offers an explicit all-sources choice.
- Exclusion removes a curve from active selection and all current Plot, Legend, Threshold, Selected Data XLSX, and plotted-data projections. It does not remove the curve from the normalized dataset.
- Restore returns the exact curve to the active dataset unselected. A later append with the same specimen/reagent labels receives a different curve ID and does not inherit an earlier exclusion.
- Analysis XLSX schema 7 stores `excludedCurveIds`, keeps the complete imported dataset, and exposes an `ExcludedCurves` review sheet. Schemas 1-6 migrate with no excluded curves.
- `selectedCurveIds` and `excludedCurveIds` must be disjoint. Unknown, duplicate, or contradictory IDs make an Analysis XLSX restore invalid.
