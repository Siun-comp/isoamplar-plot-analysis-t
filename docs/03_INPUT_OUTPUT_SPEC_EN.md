...
- Analysis XLSX schema 6 is a web-app restore file containing the full imported dataset and settings, including Analysis labels, Selection Sets, Common/per-reagent Threshold configuration, and legend/export settings; it is separate from plotted-data CSV and report-style chart workbooks.
...
| IO-106 | Analysis XLSX export | Export schema 6 with the complete imported dataset and common/per-reagent Threshold mode, drafts, applied values, enabled states, rule IDs, and independent preview/export visibility. A visible `ReagentThresholds` sheet supports review; hidden restore JSON remains authoritative. Schema 5 restores as Common mode. Derived results are recomputed from raw data. | FR-017, FR-023 | AC-PCR-033, AC-PCR-034, AC-PCR-037, AC-PCR-058, AC-PCR-064 |
